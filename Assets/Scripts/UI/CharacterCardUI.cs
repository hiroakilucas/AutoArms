...
public class CharacterCardUI : MonoBehaviour
{
    ...
    public const float CardHeight = TopMargin + PortraitBoxSize + Gap + LevelXpHeight + BottomMargin;

    private const float PortraitBoxSize = 170f;
    private const float NameStripHeight = 32f;
    private const float LevelXpHeight = 46f;
    ...
    private float BuildPortraitBox(UITheme theme, bool isEnabled, float y)
    {
        ...
        boxRt.sizeDelta = new Vector2(PortraitBoxSize, PortraitBoxSize);
        boxRt.anchoredPosition = new Vector2(0f, y);
        var boxImg = boxGo.AddComponent<Image>();
        boxImg.sprite = UIShapeUtil.RoundedRect(bgColor, 16f);
        ...
        if (profile.previewIcon != null)
        {
            ...
            // Dessaturação aproximada via tint (sem shader custom) — bloqueado fica visivelmente
            // acinzentado/escurecido, estilo personagem não desbloqueado do Brawl Stars.
            if (!isEnabled) portraitImg.color = new Color(0.42f, 0.42f, 0.42f, 1f);
        }
