...
Fix: nova constante `CombatPlayer.ThrowFlightDuration = 0.35f` (era `0.45f`, hardcoded 2x no case `ThrowWeapon`) — fica dentro da janela de saída válida do **1º loop** (entre 0.3s e 0.4s), então `SetIdle(true)` cai exatamente quando a transição já está disponível, sem precisar de um 2º loop. Não mexi nos outros 3 usos de `0.45f` (`BoomerangReturnFlight` linha ~293, `Repulse` linhas ~1924/1929) — mesmo risco em teoria (nenhum deles foi reportado como quebrado ainda), mas fora do escopo deste fix; revisar se o mesmo bug aparecer lá.
