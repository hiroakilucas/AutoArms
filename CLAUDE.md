...
**Reusa o `CombatEventType.Saboteur` já existente** (mesmo evento da skill Saboteur) em vez de criar um novo — o visual pedido pelo usuário ("a arma deve cair do hud para o chão igual faz o saboteur") já é exatamente o que esse evento faz: popup "SABOTAGE!" + queda em pêndulo do ícone até o chão. Como a arma destruída nunca é a equipada, `CombatPlayer`'s case `Saboteur` não precisa de nenhum ajuste pra desequipar a mão — mesmo comportamento da skill Saboteur original (pré-luta, ninguém armado ainda).

**Sem pausar a luta**: novo campo `CombatEvent.isMidFight` (`true` só quando emitido por Sabotage, default `false` no Saboteur pré-luta) — `CombatPlayer`'s case `Saboteur` só executa o `yield return new WaitForSeconds(0.6f)` final quando `!evt.isMidFight`. A pausa fazia sentido pro Saboteur (acontece uma vez, antes de qualquer turno, dá tempo do jogador ler o popup antes da luta começar de verdade), mas pra Sabotage (que pode disparar em qualquer hit, repetidamente) ela travava a luta por 0.6s a cada acionamento — pedido do usuário pra deixar a arma cair em paralelo (`StartCoroutine(DropWeaponFromHud)`, já fire-and-forget) sem segurar o resto dos eventos.

### Flash Flood

Primeira Super (categoria `SkillCategory.Super`, `usesPerFight = 1`) implementada — checada no **topo de `SimulateTurn`**, antes até de Thief (item 0): se `attacker.HasSkill("Flash Flood")`, `attacker.flashFloodUsesRemaining > 0`, `attacker.weaponLoadout.Count >= 3` (a arma equipada nunca entra nessa lista — ver `PlayerState`/`BuildState`, então "excluindo arma em mão" já vem de graça, sem checagem extra) e `attacker.hitSpeed > 0f` (Monk guarda, nunca ataca — mesma checagem de Throw/Melee), **17% de chance por ação** (não por luta — rolado de novo em toda ação do atacante enquanto o uso não for consumido). Ao triggar, consome **a ação inteira do turno**: sem Thief/pickup/swap/throw normal/melee no mesmo turno, igual ao guard de stun (`StunSkip`).
...
Terceira Super implementada (categoria `SkillCategory.Super`, `usesPerFight = 1`, novo `PlayerState.piledriverUsesRemaining`) — checada no topo de `SimulateTurn`, logo depois de Haste (item "0c"): se `attacker.HasSkill("Piledriver")`, `attacker.piledriverUsesRemaining > 0`, `attacker.hitSpeed > 0f` e **17% de chance por turno** (valor fixo do jogo original, diferente do 23% de Haste e do 17% de Flash Flood — coincide com Flash Flood só por acaso). Não exige nenhuma arma (igual a Haste).
...
Quarta Super implementada (categoria `SkillCategory.Super`, `usesPerFight = 1`, novo `PlayerState.netUsesRemaining`) — checada no topo de `SimulateTurn`, logo depois de Piledriver (item "0d"): se `attacker.HasSkill("Net")`, `attacker.netUsesRemaining > 0`, `attacker.hitSpeed > 0f` e **50% de chance por turno** (valor fixo do jogo original, "Usage Rate: 50%" — bem mais alto que as outras Supers, mas a skill não causa dano nenhum, é puro controle). Não exige nenhuma arma.
...
**Nota sobre a interação com Net** (mencionada na descrição original do usuário: "Net imobiliza o oponente e Fierce Brute dobra o dano do hit seguinte nesse mesmo turno"): como **Net consome o turno inteiro** (`return` logo depois de `NetThrow`, sem Thief/pickup/throw/melee no mesmo turno — ver seção **Net** acima), essa combinação descrita não acontece de fato na implementação atual — se Net ativar, o checkpoint de Fierce Brute (item "0e") nunca é alcançado naquele turno. Fierce Brute funciona normalmente de forma independente; se o usuário quiser esse combo de verdade, Net precisaria deixar de encerrar o turno ao ativar (mudança de comportamento não pedida explicitamente, fora do escopo desta tarefa).
...
3. **Turno não encerrava se Bomb matasse o defensor** — diferente de Net/Piledriver/Haste/Flash Flood (que sempre terminam o turno inteiro ao ativar), Bomb nunca consome o turno por padrão — então se a explosão zerasse o HP do defensor, `SimulateTurn` continuaria pro fluxo normal (Thief/pickup/throw/melee) contra um alvo já morto, só sendo notado pelo loop de round na ação seguinte (não chegava a quebrar a detecção do vencedor no fim da luta, mas gerava ações sem sentido contra um corpo no mesmo turno). Corrigido com um guard `if (!defender.isAlive) { Emit(TurnEnd); return; }` logo depois do loop de Supers embaralhado.
...
- [x] Bomb — 17% por turno: explosão entre 15-25 de dano em TODOS os alvos inimigos (hoje só o defensor, ver GetEnemyTargets), ignora dodge/block/crítico/STR/armor, quebra Net em quem estiver enredado (2x por luta); ver seção própria **Bomb** em Combat Systems
