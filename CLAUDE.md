...
4. **Mira em salto instantâneo ficava "estranha"/parecia travada (2026-07-06)** — `AimWeaponAt` (removido) girava a arma pro ângulo final NUM SÓ FRAME, sem transição — usuário reportou "esta estranho ainda" no 1º turno e "no segundo turno o bow fica estático apontando para frente" nos seguintes. Causa provável do "travado": como o atacante agora não corre mais (fix 1), a distância/ângulo até o defensor mal muda de turno pra turno (principalmente se o defensor também não se move muito) — reverter pro `rotationOffset` neutro entre tiros e IMEDIATAMENTE re-mirar quase no mesmo ângulo, tudo em salto instantâneo, dava a impressão de "nunca volta, fica sempre apontando pra frente", mesmo a arma tecnicamente resetando e re-mirando a cada tiro. Pedido explícito do usuário: "faça um movimento sutil em cada flecha, de baixo para frente até sair a flecha". Fix: `AimWeaponAt` removido; a mira virou parte de `PlayProjectileEffect` — antes de spawnar a flecha, gira a arma gradualmente (`Mathf.LerpAngle`, `BowDrawStartOffset = -35°` abaixo do ângulo final, `BowDrawDuration = 0.15s`, ambos chutes iniciais pra calibrar) do ângulo inicial até o ângulo de mira de verdade, só então lança a flecha — o movimento fica visível/deliberado a cada tiro, em vez de um pulo que passava despercebido.
