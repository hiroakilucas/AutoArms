...
**Centralização (2026-07-07)**: `CharacterCenterX = -1.76f` (era `-2f`, levemente descentralizado) — o personagem não fica no centro absoluto da tela, e sim no centro do espaço **à esquerda** do `CharacterPanel` (que ocupa a faixa direita, ver subseção abaixo). Derivado da câmera ortográfica da cena (`orthographicSize = 5`, aspecto 16:9, câmera em X=0 — confirmado em `01_MainMenu.unity`): meia-largura visível = `5 × (1920/1080) = 8.889` unidades de mundo. `CharacterPanel.PanelWidth = 450px` ocupa `450/1920 ≈ 23.4%` da tela à direita; o centro da faixa restante, convertido de volta pra espaço de mundo, cai em `X ≈ -1.76`. **Recalcular se `PanelWidth` ou o `orthographicSize` mudarem** — os dois valores estão documentados como constantes com a fórmula no comentário de `MainMenuCharacterPreview.cs`. (A constante ainda diz `-1.76`, calculada quando `PanelWidth` era 380px — o painel cresceu de novo depois, ver subseção abaixo; a diferença é pequena o bastante pra não ter sido recalculada, mas fica registrado aqui pra quem for mexer de novo.)

`MainMenuCharacterPreview.Start()` também chama `BuildLevelXpHud(profile)` — barra de XP fina + texto "Level X" acima dela (estilo My Brute), única barra de XP do menu (a que existia no `CharacterPanel` foi removida, ver subseção abaixo). Canvas `ScreenSpaceOverlay` próprio (sortingOrder=4, parentado em `transform`), painel pequeno (240×56px) centralizado na mesma fração de tela X do personagem (mesma fórmula de câmera acima) e ancorado a `y=0.80` (bem acima da cabeça do personagem, com folga).
...
### CharacterPanel — compacto/expandido, com Skills e Armas (restaurado 2026-07-07)
...
- **Dois filhos de `Root`, sempre ambos construídos, um ativo por vez**: `Compact` (altura fixa `CompactHeight = 230px`, ancorado ao topo do Root) e `Expanded` (`Stretch` do Root inteiro). Cada um com seu próprio `CanvasGroup` — a troca é um crossfade de alpha (`FadeDuration = 0.18f`, coroutine `CrossFade`), não duas telas desconectadas.
  ...
  - **Expanded, abaixo do bloco de info** (divisor dourado fino + `ScrollRect`): duas seções, **HABILIDADES** e **ARMAS** (título dourado 20pt + lista), cada skill/arma como uma linha ícone (56×56, sobe a cadeia `previousTier` se o tier atual não tiver sprite — mesmo padrão de `WeaponHandler.EquipSpecific`) + nome (18pt) — `BuildIconNameRow`, compartilhado pelas duas seções. Mensagem de "nenhuma equipada" quando a lista está vazia. Cada seção usa um sub-container com seu próprio `VerticalLayoutGroup`+`ContentSizeFitter` (`MakeListContainer`) — o `VerticalLayoutGroup` externo do scroll trata cada sub-container como um bloco só, dimensionado pela soma das próprias linhas.
- **Clique-fora-recolhe, sem código extra pra "detectar fora"**: o `Button` de `Collapse()` fica no próprio background do `Expanded` (elemento mais profundo da hierarquia) — não há mais botões de aba aqui (lista vertical única, sem abas, "o que ficou mais legível dado o tamanho de fonte", conforme pedido). Um clique em qualquer linha/ícone/texto sem `Button` próprio **borbulha** via `ExecuteEvents.GetEventHandler<IPointerClickHandler>` até achar o `Collapse` do fundo. Arrastar pra rolar o `ScrollRect` de Skills/Armas não dispara clique nenhum (Unity distingue drag de click), então rolar a lista não recolhe o painel por engano.
- `Setup(holder, theme)` já chama `RefreshAll()` uma vez; `Expand()` chama de novo por segurança/frescor dos dados.
- **`AttributePipBar` (`Assets/Scripts/UI/AttributePipBar.cs`)** — componente reutilizável (classe simples, não `MonoBehaviour`) pras 3 linhas de atributo (STR/AGI/SPD), usado nos dois blocos de info (Compact e Expanded): badge circular (`UIShapeUtil.RoundedRect` com raio = metade do lado, 36×36px) mostrando o valor numérico, seguido de uma fileira de 10 pips/blocos preenchidos representando o progresso dentro da faixa atual de 10 pontos. Cor do badge+pips muda a cada múltiplo de 10 (`tierIndex = ((valor-1)/10) % 4`, ciclando entre `secondaryButton → currencyGold → primaryActionAlt → danger`). `Build(rowContainer, theme, label)` só preenche o conteúdo de um container já dimensionado pelo chamador. `SetValue(int)` atualiza badge+pips.
