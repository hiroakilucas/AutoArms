...
3. **Ainda fazia "Slashing" (giro de espada) mesmo parado** — depois do fix (1), o corpo do atacante continuava disparando o trigger `Slashing`/`SlashingDagger` (`SwingTrigger`/`SetTrigger`) nos mesmos 6 pontos (Hit/Dodge/Block/Counter-Reversal, pet e principal), visualmente errado pra um arco parado no lugar. Usuário não tem (ainda) um clipe de "erguer o arco" dedicado — pedido explícito: só a ARMA deveria se mexer, sem nenhum swing de corpo. Fix: `IsRangedWeapon(attacker)` também pula o bloco de `SwingTrigger`/`SetTrigger`/`SetSpeed` nesses 6 pontos (mantendo o `WaitForSeconds(slashHalf)` de antes e depois, pra flecha/pose ainda sincronizarem no mesmo ritmo) — o corpo fica em Idle o tempo todo, só a arma (rotação + attackSprite) e a flecha se movem.
4. **Mira em salto instantâneo ficava "estranha"/parecia travada (2026-07-06)** — `AimWeaponAt` (removido) girava a arma pro ângulo final NUM SÓ FRAME, sem transição — usuário reportou "esta estranho ainda" no 1º turno e "no segundo turno o bow fica estático apontando para frente" nos seguintes. Causa provável do "travado": como o atacante agora não corre mais (fix 1), a distância/ângulo até o defensor mal muda de turno pra turno (principalmente se o defensor também não se move muito) — reverter pro `rotationOffset` neutro entre tiros e IMEDIATAMENTE re-mirar quase no mesmo ângulo, tudo em salto instantâneo, dava a impressão de "nunca volta, fica sempre apontando pra frente", mesmo a arma tecnicamente resetando e re-mirando a cada tiro. Pedido explícito do usuário: "faça um movimento sutil em cada flecha, de baixo para frente até sair a flecha". 1ª tentativa: `AimWeaponAt` removido, mira virou parte de `PlayProjectileEffect`, girando gradualmente (`Mathf.LerpAngle`) de um ângulo relativo (`aimAngle - 35°`) até o ângulo de mira de verdade.
5. **1ª tentativa ainda tinha 2 problemas (2026-07-06)**: (a) usuário reportou "a primeira animação do primeiro turno está diferente do segundo turno" — o ângulo de PARTIDA do "erguer" (`aimAngle - 35°`) é relativo à mira (dinâmico), mas o ângulo em que a arma ficava parada entre tiros vinha do `rotationOffset` genérico do personagem (fixo, do equip) — se os dois não coincidissem, o 1º tiro (partindo do `rotationOffset` do equip) e os seguintes (partindo de onde o tiro anterior deixou, se o revert funcionasse) podiam começar de pontos diferentes; (b) "a animação ficou muito sutil, deixe menos sutil" — só 35° de giro em 0.15s. Fix: nova constante `CombatPlayer.BowRestAngle = -90f` (reto pra baixo, mesmo significado nos 2 lados P1/P2) — usada como o único ângulo de "descanso" tanto no revert (`SetWeaponSwingPose(attacking: false)` força a arma pra esse ângulo fixo, em vez de confiar no `rotationOffset`) quanto no início do próximo "erguer" (`PlayProjectileEffect` sempre parte de `BowRestAngle`, não mais de um offset relativo à mira) — garante que todo tiro comece exatamente do mesmo jeito. `BowDrawDuration` dobrada pra `0.3s` e o giro agora cobre até 90°+ (reto pra baixo até o ângulo de mira) em vez de só 35° — bem mais visível.
