...
`Default < Weapons2 < Characters2 < Weapons < Characters` (ordem fixa do projeto, fundo→frente). Durante um turno (`CombatPlayer.ExecuteEvent`, casos `TurnStart`/`TurnEnd`, via `PlayerCombat.SetAttackerLayers`/`RestoreDefaultLayers`), body/arma de atacante e defensor são mapeados nessas 4 layers pra reproduzir, fundo→frente:

`Background < Arma do atacante < Corpo do atacante < Arma do defensor < Corpo do defensor`

- Arma do atacante → `Weapons2` (mais atrás das 4)
- Corpo do atacante → `Characters2`
- Arma do defensor → `Weapons`
- Corpo do defensor → `Characters` (mais à frente das 4)

Regra por personagem: a própria arma sempre atrás do próprio corpo. Regra cruzada: a arma do atacante nunca sobrepõe nada do defensor (fica atrás até do corpo dele); a arma do defensor fica na frente do atacante inteiro (corpo+arma), mas ainda atrás do próprio corpo. Note que `Characters`/`Characters2` agora identificam **defensor/atacante** (não atacante/defensor, como antes de 2026-07-04) — só a ocupação por turno importa; outros sistemas que reusam essas 2 layers (pets, ver `PetCombatController.SetDepthLayer`) não fixam suposição nenhuma sobre qual papel ocupa qual layer. Restaurado (`Default`) em `TurnEnd`.
...
`Blunt`, `Long` e `Ranged` contribuem 0 em todas essas tabelas — `Blunt` só importa pra Lead Skeleton (-15% dano recebido, ver `WeaponData.IsBlunt`) e pra **não** receber o bônus de Weapon Master; `Long` não tem tabela própria — o bônus de Counter Rate/Reversal de cada arma "Long" é definido manualmente por asset (`reversalBonus`/skills futuras), não por uma constante global; `Ranged` (=7) adicionado ao enum para uso futuro com Arco e Flecha — sem lógica de combate associada ainda.
