...
Flow:
1. **Atacante com Hideaway**: nunca desequipa — `Unequip()`/`UnequipPermanent()` nem são chamados, a arma fica na mão e no loadout o turno inteiro (ver **Hideaway** abaixo). **Sem a skill** — tag `Thrown` (`HasType`, não `==` exato — uma arma pode combinar Thrown com outra tag): `Unequip()` only (stays in loadout, comes back next cycle). **All others**: `UnequipPermanent()` = `Unequip()` + `loadout.RemoveCurrentWeapon()` (removed from runtime loadout for this combat).
2. Create `FlyingWeapon` GameObject with the weapon's `inHandSprite` — uma réplica independente; com Hideaway, o objeto na mão continua existindo em paralelo (não foi destruído pelo `Unequip` no passo 1). `localScale = Vector3.one * weaponData.scale` for **all types** (same scale as the in-hand sprite).
...
5. On landing: **defensor com Hideaway** primeiro tem chance de **bloquear** o arremesso (ver **Hideaway** abaixo); se não bloquear, 80% hit (weapon damage + STR + Hurt + knockback), 20% miss — defender plays `DodgeLeap` (same animation as dodge) + gray "MISS!" popup.
6. After hit/miss: **sem Hideaway**, fica desarmado até o próprio `TurnStart` do próximo turno dele, que já tem o check normal de 40% pra pegar arma — mesmo padrão de qualquer outro turno desarmado. Era **40%** de chance de re-equipar imediatamente aqui (`CombatEventType.WeaponEquipped`, removido), mas isso fazia o personagem ocasionalmente "equipar" uma arma já no fim do turno (depois do hit/miss do arremesso), sem nenhuma ação visível correspondente — bug real reportado pelo usuário: pickup deveria sempre acontecer no início do turno, nunca no meio/fim. **Com Hideaway**, esse passo não se aplica — ele já está armado desde o passo 1, sem nenhum gap.
...
Skill com 3 efeitos, todos checados vivo via `HasSkill("Hideaway")` (sem flag cacheada em `ApplySkillStats`, mesmo padrão de Iron Head/Determination/Resistant) — mecânica oficial do My Brute, os 3 valores batem com a skill original:

1. **+25% chance de bloquear arremessos** (`ThrowBlockChance(defender)`, novo) — mecânica que **não existia antes**: arremesso só tinha 80% hit/20% miss, sem nenhum Block. Quem não tem a skill tem **0%** de chance (não bloqueia arremesso nunca); quem tem, **25%**. Checado em `SimulateThrow` **antes** do `Roll(0.80f)` de acerto — se bloquear, emite `CombatEventType.Block` com `isThrow = true` (mesmo campo do `Hit`) e pula o hit/miss inteiramente (sem dano). `CombatPlayer`'s case `Block` agora pula o swing/reposicionamento do atacante quando `evt.isThrow` (ele já está desarmado, acabou de arremessar — sem sentido repetir uma animação de swing de melee), igual ao `Hit` já fazia.
2. **Arremesso forçado a 100% sempre que estiver armado** (redefinida pelo usuário — a versão original tinha 50% fixo via `ThrowChance()`, errado): checado no **topo de `SimulateTurn`** (item 3, antes do throw-ou-melee normal do item 4), **sem rolar `ThrowChance()` nem checar tag nenhuma** — `attacker.HasSkill("Hideaway") && attacker.currentWeaponData != null` já é suficiente. O personagem **nunca vai até o defensor pra melee** enquanto estiver com arma na mão, independente do tipo dela — arremessa sempre do lugar onde está. Sem arma (`currentWeaponData == null`), Hideaway não tem efeito nenhum: age normal (40% de pegar arma no início do turno, ataca desarmado se não pegar) — a skill só importa quando já está armado. `ThrowChance()` não soma/considera Hideaway mais — só decide o throw de quem não tem a skill.
   - **Combo depois do throw também é throw, nunca melee** — `SimulateHideawayThrowCombo` (não reusa `SimulateComboLoop`, que faz melee) rola a mesma fórmula/decaimento de `ComboChance()` a cada hit extra e, se passar, chama `SimulateThrow` de novo (em vez de `SimulateHitWithDetermination`). **Bug corrigido**: a 1ª versão chamava `SimulateComboLoop` (melee) depois do throw forçado — Hideaway ainda acabava atacando corpo a corpo no combo, contradizendo a própria skill ("nunca vai pro melee enquanto tiver arma"), bug real reportado pelo usuário. Sem "interrupted" — `SimulateThrow` não tem Counter/Iron Head, nada nele cancela o resto da sequência. Sem Hideaway, throw normal continua sem combo nenhum (`return` imediato depois de `SimulateThrow` + `TurnEnd`, como sempre foi).
   - **Ação extra por Speed**: cada ação extra é só outra chamada de `SimulateTurn` (mesmo padrão de sempre, ver **Speed System**) — o forçamento do throw roda de novo do zero em cada uma, automaticamente, sem precisar de nenhum estado extra entre ações.
   - **Troca de arma não muda**: Hideaway não influencia a chance de pickup (40%) nem a de swap (40%, ver **Troca de Arma** abaixo) — só garante que, uma vez com arma em mãos, ela será arremessada.
3. **A arma nunca sai da mão de quem arremessa** (`keepsWeapon` em `SimulateThrow`/`CombatPlayer`'s case `ThrowWeapon`) — `attacker.currentWeaponData` **não** é zerado e ela também não sai de `weaponLoadout`, mesmo sem a tag `Thrown`; visualmente, `weaponHandler.Unequip()`/`UnequipPermanent()` nunca é chamado, então a arma continua equipada e visível na mão o turno inteiro — só uma réplica (`FlyingWeapon`, já existente) é arremessada de fato. Sem a skill, a regra de sempre continua: fica desarmado até o próprio `TurnStart` seguinte (mesmo padrão de qualquer outro turno desarmado) e só `Thrown` sobrevive no loadout.
   - **Bug corrigido**: a versão original só ajustava o lado do `weaponLoadout` (dado/simulador) mas esquecia de espelhar a mesma condição no visual — `CombatPlayer`'s case `ThrowWeapon` chamava `UnequipPermanent()`/`Unequip()` incondicionalmente, então mesmo um Hideaway holder ficava com a mão visualmente vazia depois de arremessar (e o ícone desaparecia do `WeaponHUD` se a arma não fosse `Thrown`), apesar do simulador já saber que ele continuava armado. Corrigido pulando o `Unequip`/`UnequipPermanent` inteiramente quando `keepsWeapon` é `true`, nos dois arquivos.
...
`TurnStart, RunToDefender, ThrowWeapon, PickupWeapon, WeaponSwap, Thief, Hit, Counter, Reversal, Dodge, Block, Miss, Disarm, WeaponDrop, ShieldDisarm, ShieldDrop, HealthChanged, SpeedBonus, TurnEnd, CombatEnd`
...
| Hideaway | Não altera nenhum stat em `ApplySkillStats` — checada vivo via `HasSkill("Hideaway")` em `ThrowChance`/`ThrowBlockChance`/`SimulateThrow` (ver **Hideaway** abaixo), mesmo padrão de Iron Head/Determination. |
...
- [x] Hideaway — +25% chance de bloquear arremessos (mecânica nova, sem essa skill a chance é 0%), arremesso forçado a 100% sempre que armado (nunca vai pro melee, combo continua do lugar onde está), a arma nunca sai da mão de quem arremessa; ver seção própria **Hideaway** em Combat Systems
