...
```
00_Login → (ChooseFirstCharacter, só conta sem personagem) → 01_MainMenu → 02_SelectCharacter → 01_MainMenu → 05_SelectOpponent → 04_CombatScenePVP
```

- `ChooseFirstCharacter` — **Criada (2026-07-24), onboarding.** Carregada por `LoginController.OnAuthSuccessRoutine` sempre que a conta loga sem NENHUM personagem no roster (`RosterService.ListOwnedCharacterDocsAsync`) — cobre sign-up, sign-in, Google e auto-login por igual; conta que já possui personagem nunca vê esta tela. Grid 1×4 horizontal com os 4 templates "normais" pré-autorados (Medieval Warrior, Medieval Warrior Girl, Citizen 1, Citizen Women 2) — tap pra destacar (`PressableCard.cs`) + botão "Confirmar" separado, que chama a Cloud Function `grantStarterCharacter` (`functions/src/grantStarterCharacter.ts`, ver ARQUITETURA.md "Modelo de roster multi-personagem"): servidor concede o personagem (stats fixos por template) e sorteia a 1ª skill entre 2 candidatos ponderados pelos odds reais do catálogo, sem escolha manual. Handoff reaproveita o mesmo `PendingCharacterSelection` → `02_SelectCharacter` do case opening, sem nenhum código novo nessa tela. `Medieval Warrior.asset` não é mais caso especial (`isPlayable=false`, igual aos outros 71) — jogabilidade só vem de instância possuída no roster. Ver `ChooseFirstCharacterController.cs`.
...
- `06_Loja` — **Criada (2026-07-20), Fase 1/placeholder (ver MONETIZACAO.md).** Tela informativa/de compra (fora do fluxo de partida) construída 100% via código (mesmo padrão de `03_Arsenal`: só Main Camera + um GameObject com `ShopController`, sem hierarquia de UI pré-colocada). **Layout em sidebar (2026-07-20, revisado — era barra horizontal de abas + grid vertical no topo):** 5 abas (Diamantes/Desbloqueios/Passes/Progressão/Personagens) numa barra lateral VERTICAL à esquerda (`BuildSidebar`, `VerticalLayoutGroup`, estilo Brawl Stars); cards de cada aba rolam HORIZONTALMENTE à direita (`BuildScrollView`/`RebuildGrid`, `GridLayoutGroup.Constraint.FixedRowCount`) — **2 linhas** na aba Diamantes (8 pacotes), **1 linha** nas demais. Cards grandes, preenchendo quase toda a altura da região disponível — `ShopCardUI.Build` recebe a altura real do card (`cardHeight`) e escala ícone/paddings/fontes proporcionalmente (`scale = cardHeight/420`); a altura do subtítulo é calculada a partir do espaço realmente sobrante entre título e preço (não uma constante escalada — evita a caixa de subtítulo sobrepor o cluster de preço/status/comprar). Preços/itens hardcoded em `ShopController.BuildItemData`, espelhando MONETIZACAO.md seções 1-5. Cards (`ShopCardUI.cs`) usam retângulo cinza placeholder no lugar do ícone, exceto a aba Diamantes (usa `Diamond.png`, arte já final); aba Personagens usa as cores de raridade do `UITheme` (rarityRare/rarityLegendary/rarityImmortal) no placeholder. Clicar em "Comprar" ainda **não** faz um pagamento de verdade (sem gateway real, Fase 8) — mas desde 2026-07-21 o RESULTADO da compra (diamante creditado, desbloqueios/passe/progressão liberados) persiste de verdade em `users/{uid}` (`WalletService.AddDiamondsAsync`/novo `ShopStateService.cs`), em vez de só um contador local em memória perdido ao sair da cena. `ShopController.Start()` carrega esse estado ao abrir a Loja; `LoginController.LoadEconomyRoutine`/`MainMenuController.RefreshEconomyOnMenuLoad` TAMBÉM carregam (bug real corrigido — sem isso, desbloqueios/passe/progressão só valiam em combate depois de visitar a Loja nesta sessão). **Aba Personagens (2026-07-23) — única aba com compra de verdade, validada server-side**: sistema de compra de personagens/case opening via Cloud Function `purchaseCase` (infra Node/TS criada do zero nesta data, `functions/` na raiz do repo) — sorteio, concessão do personagem (`users/{uid}/characters/{id}`) e débito de diamante acontecem todos no servidor, nunca no cliente. 3 cards cash (Raro/Legendary/Imortal, sem repetição, limite de compras 10/3/1 **por jogador**, não estoque global). Ao comprar, abre `CaseOpeningPopup` (roleta horizontal estilo CS:GO, `Assets/Scripts/UI/CaseOpeningPopup.cs`/`CaseReelSpinner.cs`) já com o resultado decidido pelo servidor. Ver ARQUITETURA.md "Modelo de roster multi-personagem" — personagem concedido já é persistido de verdade e jogável (migração de `02_SelectCharacter`/`SelectedProfileHolder` concluída 2026-07-24).

**"Case Geral" removido / "Próximo Personagem" virou compra real (2026-07-26/27)**: o antigo 4º card "Case Geral" (diamante, pool de todas as raridades) foi removido — substituído pelo card "Próximo Personagem" (antes só decorativo, seção 6 de MONETIZACAO.md, nunca debitava moeda de verdade), que agora é uma compra real em Coins via nova Cloud Function `purchaseNextCharacter` (`functions/src/purchaseNextCharacter.ts`), com o MESMO sorteio ponderado entre as 5 raridades que o Case Geral usava (`DEFAULT_TIER_WEIGHTS`, reaproveitado via `functions/src/caseRoll.ts` — extraído de `purchaseCase.ts` junto de `functions/src/grantCharacter.ts`, pra não duplicar lógica entre as duas functions). Preço escala por um contador PERSISTIDO por jogador (`users/{uid}.nextCharacterPurchaseCount`, não mais por pacote fixo): 25/50/100/200/400/800/1200/1400/1600/1800/2000/2200, depois +200/compra (tabela final em MONETIZACAO.md §6). `ShopController.CharacterSlotCost` espelha essa tabela só pra exibição. **Bug visual corrigido no mesmo pacote de mudanças**: `CaseOpeningPopup.BuildReelSlotIds` preenchia os slots de giro a partir de `reelPoolCharacterTypeIds`, que é sempre homogêneo por raridade (o pool elegível DENTRO do tier já sorteado pro prêmio) — certo pros 3 cards cash (raridade travada), errado pra "Próximo Personagem" (deveria misturar as 5 raridades no giro). Novo parâmetro `CaseOpeningPopup.Show(..., weightedRarityFill: bool)` resolve os dois casos sem duplicar o popup. Acessível pelo botão "LOJA" em `01_MainMenu` (mesma coluna de atalhos de Chibers/Arsenal/Replay, ver `MainMenuController.BuildLojaMenuButton`).
...
| `CharacterRebirthSettings` | `Assets/Resources/CharacterRebirthSettings.asset` | Novo (2026-07-26, economia invertida no mesmo dia) — `coinRewardPerLevel` (10, era `coinCostPerLevel`), único campo, usado por `CharacterPanel.OnRebirthClicked` pro botão "RENASCIMENTO" (Reset Nível 10+ — reseta pro Level 1, GRATUITO, CREDITA `nível × coinRewardPerLevel` moedas via Cloud Function `rebirthCharacter`, concede skills/armas/pets pela raridade). Asset separado de `CharacterResetSettings` de propósito (dois sistemas econômicos deliberadamente separados, mesmo os dois creditando hoje — só o Renascimento tem gate de Level 10 e concede itens em vez de limpar). Espelhado no servidor em `functions/src/rebirthCharacter.ts` (`REBIRTH_COIN_REWARD_PER_LEVEL`). |
...
- `CombatResultPanel` — Screen-space overlay panel shown 0.8s after combat ends. Shows result title (gold/red), XP gained, animated blue XP bar, current XP / required, level, battles remaining, "Continuar" button (→ `01_MainMenu`). Level-up: bar animates to full, "LEVEL UP!" text pulses with sin-wave scale, bar resets to new level's progress, **then `ShowLevelUpChoice` always opens and blocks "Continuar" until a choice is made** (`continueBtn.interactable = !didLevelUp`). Ensures `EventSystem` exists; uses `GraphicRaycaster` on canvas (added to `CombatHUD.CreateCanvas`) for click detection. Overlay has `raycastTarget = false` so it doesn't block the button.
  - **`ShowLevelUpChoice` reescrita por completo (2026-07-21, pedido do usuário — era 2 cartas fixas com pesos 60/30/10 por TIPO)**: N caixas em layout de PIRÂMIDE (fileira de cima = metade arredondada pra baixo, fileira de baixo = o resto — 2/3 caixas → 1/2; 4 → 2/2; 5 → 2/3), N = `PlayerProgressionState.LevelUpBoxCount()` (2 base + 1 por Slot de Skill comprado na Loja, até 5). **Caixa 1 sempre status base** (HP/STR/AGI/SPD, `LevelUpEngine.DrawBaseAttributeOption`) — nunca skill/arma/pet. **Caixas 2+**: sorteio ponderado pelos odds REAIS de cada skill/arma/pet elegível (`SkillData.odds`/`WeaponData.dropOdds`/`PetData.odds`, aplicados via `Tools > AutoArms > Apply My Brute Odds`/`OddsApplier.cs`) via `LevelUpEngine.DrawWeightedOption` — roll 0-100 contra a soma cumulativa dos itens elegíveis; a fatia residual (sem cobertura) cai pro mesmo pool de status base da Caixa 1, nenhuma caixa fica vazia. SEM repetir skill/arma/pet já sorteado nas caixas ANTERIORES do mesmo level-up (pools filtrados a cada sorteio seguinte); atributos podem repetir. Cada caixa gira uma "roleta" (`LevelUpReelSpinner.cs`, ícone cicla por um pool de sprites de todo o jogo antes de travar no resultado real) travando em sequência da esquerda pra direita; "Escolher" e o popup do ícone ficam desabilitados até travar. Clicar no ícone (skill/arma/pet, não Atributo) abre o MESMO popup de detalhe do menu principal (`ShowSkillDetail`/`ShowWeaponDetail`/`ShowPetDetail`, via um `CharacterPanel` extra instanciado com `hideCompact: true` — só aparece via um botão quadrado próprio, nunca o bloco Compact padrão). Painel também tem um contador de diamante (`PlayerEconomyState.Diamonds`) e um botão "Novo Sorteio" (canto inferior direito da TELA, fora da janela de escolha) que gasta diamante — preço progressivo 50/100/200 (1º/2º/3º uso), limite de 3 por level-up, via `WalletService.SpendDiamondsAsync` — e refaz o sorteio + a roleta das N caixas do zero. Logs `[LevelUp] ERRO: SkillDatabase não encontrado ou vazio` if `skillDatabase` is null/empty. Picking a card calls `ApplyBonus` (mutates `profile` directly) then carrega `01_MainMenu` direto (não passa mais pelo botão "Continuar").
    - **Modo teste (`ShowAllOptionsForTesting`, const em `CombatResultPanel.cs`, DESLIGADA em 2026-07-21 — era `true`)**: em vez de sortear as caixas, chama `ShowAllOptionsChoice` — grade rolável (`ScrollRect` + `GridLayoutGroup`) com as opções disponíveis: 4 atributos + skills da `SkillDatabase` que (a) ainda não foram escolhidas e (b) têm `icon != null`. Religar só temporariamente pra voltar a testar ícone de skill um por um (ver nota abaixo).
...
`MainMenuCharacterPreview` instancia o character prefab pra exibição, então imediatamente `DestroyImmediate` `PlayerCombat`, `WeaponHandler` e `MovementController` — deixando `Animator`+`AnimationController` em idle state (`AnimationController` **não é mais destruído**, 2026-07-08 — precisa dele pra reagir a clique, ver **Reação de Clique** abaixo). Escala o personagem instanciado por `PreviewScaleFactor = 0.82f` (`profile.scale * PreviewScaleFactor`) — como o root do prefab fica nos pés (mesmo pressuposto de `CombatSceneLoader`/`RandomSpawnPosition`), escalar em torno da própria transform mantém os pés no lugar sem precisar ajustar a posição. (`CharacterSelectController` tem seu PRÓPRIO preview, via `RenderTexture` — ver **Portrait Preview (02_SelectCharacter)** logo abaixo — usa a mesma técnica de reação a clique, mas não este world-space direto, ver histórico no CHANGELOG.)

**Reação de clique (2026-07-08, pedido do usuário)**: personagem central reage com Hurt ou Slashing (sorteio 50/50) ao ser clicado, igual ao portrait de `02_SelectCharacter` — mesmo componente compartilhado `CharacterPreviewReaction` (`Assets/Scripts/UI/CharacterPreviewReaction.cs`) nos dois lugares, só o gatilho de clique muda: aqui o personagem é world-space de verdade (não uma `RenderTexture`), então o clique é detectado via `BoxCollider2D` + `OnMouseDown` (mensagem nativa da Unity, funciona independente de `EventSystem`/`GraphicRaycaster` — caminho de input totalmente separado do clique em UI). `MainMenuCharacterPreview.BuildClickReaction` dimensiona o collider a partir dos bounds REAIS de todos os `Renderer`s do personagem (soma das partes do sprite, Spriter2UnityDX) — não um tamanho fixo chutado, já que cada personagem tem proporções diferentes.

### Portrait Preview (02_SelectCharacter) — RenderTexture + clique reage com Hurt/Slashing (2026-07-08)

O portrait grande de `02_SelectCharacter` (área central-esquerda, dentro da moldura dourada) mostrava só `PlayerProfile.previewIcon` (ícone estático) — decisão tomada depois de uma tentativa anterior de instanciar o prefab direto na cena falhar (CHANGELOG 2026-07-08: "preview em world-space... dependia de sorting/profundidade de câmera nunca validado"). A causa raiz dessa falha nunca tinha sido diagnosticada: `SelectionOverlay` é um Canvas `ScreenSpaceOverlay` (mesmo Canvas de todo o resto da UI da cena) — um Canvas desse tipo **sempre** desenha por cima de QUALQUER coisa renderizada por uma câmera de mundo, não importa profundidade/sorting layer/order — um personagem instanciado normalmente na cena fica, por definição, atrás do fundo sólido do overlay, sempre, sem exceção.

Pedido do usuário (personagem em Idle no lugar do ícone estático, reagindo com Hurt/Slashing ao clicar) exigia um personagem de verdade (Animator tocando), não um sprite. Fix: em vez de instanciar em world-space na cena principal, `CharacterSelectController.SpawnCharacterPreview` instancia o prefab **longe do resto da cena** (`PreviewWorldPos = (300, 0, 0)`, fora do frustum da Main Camera) e uma **câmera dedicada** (`CharacterPreviewCamera`, criada em `BuildPortraitPreview`, ortográfica, `clearFlags = SolidColor` com alpha 0, `targetTexture` = uma `RenderTexture` 768×768 criada em runtime) grava essa área numa textura — exibida num `RawImage` (não mais `Image`) dentro do MESMO Canvas/hierarquia de sempre, num Rect Transform próprio (ver **Frame de tela cheia + Rect Transform do Portrait** abaixo — não mais amarrado à moldura dourada, que virou tela cheia). Por ser conteúdo de UI de verdade (uma textura num RawImage), respeita a ordem normal do Canvas — sem depender de nenhum truque de sorting/profundidade de câmera de mundo, eliminando de vez a classe de bug documentada no CHANGELOG.

- **Instância**: `SpawnCharacterPreview(profile)` reaproveita a mesma técnica de `MainMenuCharacterPreview` (`DestroyImmediate` de `PlayerCombat`/`WeaponHandler`/`MovementController`) — **mas mantém o `AnimationController`**, que alimenta o `CharacterPreviewReaction` (componente compartilhado, `Assets/Scripts/UI/CharacterPreviewReaction.cs`, ver também **Menu Character Preview** acima) adicionado ao personagem via `AddComponent`. Chamado a cada `OnCharacterSelected` (troca de card clicado) — destrói o preview anterior antes de instanciar o novo.
- **Clique → reação**: `RawImage` tem um `Button` próprio (`onClick` = `OnPortraitClicked`, que só encaminha pro `CharacterPreviewReaction.TriggerReaction()` do personagem atual) — 100% raycasting de UI normal, sem collider/raycast físico no personagem (diferente do 01_MainMenu, que usa `OnMouseDown`, já que aqui não existe um personagem "de verdade" na cena visível pro mouse — só a câmera de preview enxerga ele). `CharacterPreviewReaction` sorteia 50/50 entre `AnimationController.PlayHurt(0.3f)` e `AnimationController.PlaySlash(0.4f)` (novo) — este último dispara o trigger `Slashing` (mesmo Any State→Slashing do combate, condição real é só `Running==false` + trigger, não depende de estar em Idle) e sai do estado via o mesmo toggle de bool `JumpStart` já usado pra escapar de Slashing fora de um `TurnEnd` normal (ver **Bug de animação travada** em Counter/Reversal, acima) — sem tocar em `MovementController` (removido), então não desloca o personagem, só o pulo/hop local de escape do state machine.
- **Perf**: câmera de preview é desativada (`previewCameraGo.SetActive(false)`) junto do resto do overlay em `HideOverlayAfterDelay` (ao fechar o painel de detalhe) e reativada em `OnCharacterSelected` — evita renderizar uma textura que ninguém está vendo enquanto o grid está em foco.
- **Enquadramento automático (2026-07-08, substitui offset/tamanho fixos chutados)**: `FitPreviewCharacter()`, chamado a cada `SpawnCharacterPreview`, centraliza o personagem no eixo Y da câmera de preview e recalcula `Camera.orthographicSize` a partir dos bounds REAIS de todos os `Renderer`s do personagem (`+15%` de folga, `PreviewFitMargin`, piso `MinPreviewOrthographicSize=1.5`) — antes usava uma constante de posição/tamanho fixa (calibrada só visualmente, nunca testada) que cortava a cabeça de personagens com proporção mais alta (`PlayerProfile.scale` varia por personagem; bug reportado pelo usuário: "esta cortando a cabeça dele"). Mesma técnica de bounds usada em `MainMenuCharacterPreview.BuildClickReaction` — funciona pra qualquer personagem sem calibração manual por asset.

**Frame de tela cheia + Rect Transform do Portrait (2026-07-08, pedido do usuário)**: a moldura dourada da DIREITA (ao redor do `CharacterPanel`) foi removida por completo; a moldura da ESQUERDA (antes só ao redor do portrait, `PortraitAreaLeft/Right`+`PortraitAreaBottomFraction/TopFraction`) agora cobre a TELA INTEIRA (`anchorMin=(0,0)`, `anchorMax=(1,1)`, sem offset). O `Portrait` (preview do personagem) virou independente dessa moldura, com seu próprio Rect Transform copiado 1:1 do que o usuário ajustou manualmente no Inspector: `PortraitPosX=220.47`, `PortraitWidth=440.95`, `PortraitTop=549.91`, `PortraitBottom=-108.01` — convertidos em código pra `offsetMin = (PosX - Width/2, Bottom)` / `offsetMax = (PosX + Width/2, -Top)` (fórmulas padrão de conversão Inspector→offset: `Left/Bottom` = offset direto, `Right/Top` = offset negado). Ancoragem em si não mudou — X colapsado em 0 (borda esquerda da tela), Y esticado entre `PortraitAreaBottomFraction/TopFraction` (0.10–0.92, mesma janela do `CharacterPanel`). Resultado (canvas 1920×1080): Portrait ocupa X de 0 a ~441px (encostado na borda esquerda) e Y de ~0 (base da tela) a ~444px — praticamente quadrado (~441×444), por isso a `RenderTexture` de `BuildPortraitPreview` continua quadrada (768×768) sem precisar mudar. **Dimensão recomendada pra gerar a imagem de preenchimento do frame**: 1920×1080 (a moldura agora É o canvas inteiro, reference resolution do projeto) — se for uma imagem com um "recorte"/janela pro personagem aparecer, esse recorte deve ficar aproximadamente no retângulo `X:[0,441] Y:[0,444]` (canto inferior esquerdo, origem Y de baixo pra cima) pra coincidir com onde o `Portrait` de verdade é desenhado.

**`PlayerProfile.splashArt` (2026-07-09, pedido do usuário: "algo pra eu colocar na mão" conforme cada arte de personagem é gerada)**: campo `Sprite` opcional em `PlayerProfile`, arrastado manualmente no Inspector. `CharacterSelectController.UpdateFrameArt(profile)` (chamado em `OnCharacterSelected`) troca o sprite do `Frame` (`BuildFrameBorder`, agora retorna a referência do `Image` em vez de void) pra `splashArt` quando preenchido — `Image.Type.Simple`, `preserveAspect=false` (estica pra cobrir o frame 1920×1080 inteiro, sem barras). Sem `splashArt`, mantém o retângulo dourado placeholder de sempre (`UIShapeUtil.RoundedRect(theme.currencyGold, 20f)`, `Type.Sliced`). `Medieval Warrior.asset` já tem `splashArt` apontando pra `Assets/Personagens/SplashArt/MedievalWarrior.jpg` (1344×768, já importado como Sprite); `Assasin Guy`/`Medieval Warrior Girl` seguem sem — arrastar a arte de cada um no campo conforme for gerada no Leonardo AI.
...
- **Dois filhos de `Root`, sempre ambos construídos, um ativo por vez**: `Compact` (altura fixa `CompactHeight = 230px`, ancorado ao topo do Root) e `Expanded` (`Stretch` do Root inteiro). Cada um com seu próprio `CanvasGroup` — a troca é um crossfade de alpha (`FadeDuration = 0.18f`, coroutine `CrossFade`), não duas telas desconectadas.
  ...
  - **Botão "RESETAR PERSONAGEM" (2026-07-21)**, logo abaixo de PASSIVAS (fim de `BuildSkillsAndWeapons`) — cor `danger`, popup de confirmação explícita (overlay+painel+2 botões, próprio/autocontido, mesmo idioma visual de `MainMenuController.BuildPopup`/`ShopController.ShowPassInfoPopup`) antes de executar (ação destrutiva). Confirmar reseta o `PlayerProfile` pro Level 1 — MESMA lógica de campos de `Tools > AutoArms > Reset All Profiles to Level 1` (`CharacterCreationEditor.cs`: level/xpCurrent/battlesRemaining/xpRequired + HP/STR/AGI/SPD via `CharacterCreation.GenerateLevel1Stats()` + skills/pets/armas zerados), persistido via `LocalSaveService.Save` (local + Firestore) — e credita `nível ANTERIOR × CharacterResetSettings.coinsPerLevel` (10) moedas via `WalletService.AddCoinsAsync`. Mecanismo PARALELO ao "Reset de Build" do `ROADMAP_FUTURO.md` (Fase 4, ainda não implementado — custaria diamante e manteria o nível atual, redistribuindo pontos manualmente); este é o oposto (gera moeda, volta pro Level 1), não o substitui. `CurrentProfile()` (novo helper privado, extrai a mesma expressão `_overrideProfile ?? _holder?.currentProfile` já usada em `RefreshAll`) resolve qual profile resetar.
  - **Botão "RENASCIMENTO" (2026-07-26)**, logo abaixo de "RESETAR PERSONAGEM" — feature NOVA e SEPARADA dela (a antiga continua intocada). Só habilitado com `level >= 10` e conta sincronizada (precisa de `characterId` real). 100% server-authoritative via nova Cloud Function `rebirthCharacter` (`functions/src/rebirthCharacter.ts`): reseta pro Level 1 com stats re-sorteados, CREDITA `nível × CharacterRebirthSettings.coinRewardPerLevel` (10) moedas (gratuito, sem custo — economia foi invertida numa correção de escopo no mesmo dia da implementação original), reabastece a energia de batalha pro teto, e CONCEDE N skills/armas/pets aleatórios pela raridade do personagem (Normal 1 / Uncommon 2 / Rare 3 / Legendary 4 / Immortal 5, `CharacterUnlockEngine.UnlockCountForRarity`) via o mesmo sorteio ponderado de `unlockEngine.ts`. Reroll individual pós-reset via nova `rerollRebirthGrant` (15 diamantes fixo, máx. 2 usos/item — núcleo compartilhado com `rerollUnlock` extraído pra `functions/src/rerollShared.ts`). **A revelação dos itens NÃO acontece inline** (pedido do usuário) — `CharacterPanel.ExecuteRebirthAsync` aplica o resultado e navega pra `02_SelectCharacter` (mesmo handoff `PendingCharacterSelection` do case opening, splash art em tela cheia), onde `CharacterSelectController` revela os itens em sequência reaproveitando a MESMA `CharacterUnlockRevealPanel` do reveal de case opening (canal novo `PendingRebirthReveal`, `Assets/Scripts/Data/`). Bônus da mesma tarefa: corrigido o TODO de segurança do "Novo Sorteio" do level-up de combate (`CombatResultPanel.cs`) — custo deixou de dobrar (era 50→100→200 diamantes client-side) e passou a ser fixo (15 diamantes, nova Cloud Function `rerollLevelUpBoxes`), validado/debitado sempre no servidor; o sorteio das caixas em si continua client-side (escopo deliberadamente reduzido, ver comentário no arquivo da function).
...
| `Btn_SelectCharacter` | The "Chibers" button (era "Personagem", ver CHANGELOG 2026-07-14) at anchor(0.5,0.5) pos=(−837.91, 8.18), size=(200.59, 166.73) (2026-07-14, reposicionado/reduzido); onClick → `OnCharacterButton`, sem ação própria (2026-07-07 — o `CharacterPanel` ficou sempre visível, sem depender de clique nenhum, ver subseção acima). Ganhou `UIButtonShadowStyle` (2026-07-14, outline+drop shadow) |
| `Btn_Arsenal` | Novo (2026-07-14) — mesma coluna de atalhos que "Chibers" (`CharacterCardButtonStyle`, ícone+label+badge), anchor(0.5,0.5) pos=(−837.91, −203.69) (2026-07-14, reposicionado — era ao lado de "Chibers" em y=8.18, agora abaixo dele, mesmo X), size=(200.59, 166.73). onClick → `MainMenuController.OnArsenalButton`, carrega `03_Arsenal`. Também ganhou `UIButtonShadowStyle`, pra consistência visual com os outros dois botões da linha |
| `BtnJogar` | Play button — **reposicionado (2026-07-07)**, estilo Brawl Stars: anchor(1,0) pivot(1,0) pos=(−60,60), size=(480,220), `Image.color` verde (`#48D15C`), label "JOGAR" 64pt bold. onClick → `OnPlayButton`, que agora carrega `05_SelectOpponent` (não mais direto `04_CombatScenePVP`). Ganhou `UIButtonShadowStyle` (2026-07-14, outline+drop shadow) |
...
### 03_Arsenal — Armas & Skills (novo, 2026-07-14)

Tela puramente informativa (fora do fluxo de partida) acessível pelo botão "Arsenal" de `01_MainMenu`, mostrando pro personagem selecionado (`SelectedProfileHolder.currentProfile`, mesma fonte do resto do menu) o maior tier que ele possui de CADA arma/skill do jogo — não só as equipadas. Só lê `PlayerProfile.weapons`/`skills`, nunca altera.

- **Cena minimalista**: `03_Arsenal.unity` só tem Main Camera + um GameObject `ArsenalController` (com `selectedProfileHolder`/`weaponDatabase`/`skillDatabase`/`theme` wireados no Inspector) — a tela inteira (Canvas, ScrollView, grids, botão Voltar) é construída via código em `Start()`, mesmo espírito de `CharacterSelectController`/`CharacterPanel`. Sem `EventSystem` pré-colocado — `ArsenalController.EnsureEventSystem()` cria um em runtime se não existir (mesmo padrão de `CombatHUD`).
- **`WeaponDatabase`** (novo ScriptableObject, `Assets/ScriptableObjects/Databases/WeaponDatabase.asset`) — lista os 26 `WeaponData` T1 (raiz de cada família); não existia um database de armas antes (só `SkillDatabase`, que já listava todas as skills/tiers). `SkillDatabase.skills` já cobre skills (filtrado por `previousTier == null` pra pegar só a raiz de cada família) — `ArsenalController.BuildSkillSlots` também pula **Garimpeiro**/**Magneto** por nome (2026-07-14, pedido do usuário): as duas ainda não têm mecânica implementada (`effectText` vazio), não fazia sentido aparecerem no grid antes de existirem de verdade.
- **`ArsenalSlotUI`** (`Assets/Scripts/UI/`) — componente reutilizável entre a grade de ARMAS e a de SKILLS: recebe `(UITheme theme, Sprite icon, ArsenalSlotUI.Tier tier, Action onClick = null, bool dimIcon = true)` e monta ícone + borda colorida por tier (bronze/prata/ouro via `UITheme.tierBronze/tierSilver/tierGold`, mesmos tokens do popup de detalhe do `CharacterPanel`) + estado bloqueado quando `tier == None` (não possuído). Placeholders de cor sólida — sem sprite de borda pronto ainda. `onClick` (2026-07-14) fica no próprio `Border` (cobre o slot inteiro) — funciona independente do tier/bloqueio, pra dar pra inspecionar qualquer arma/skill mesmo sem possuir. `dimIcon` (2026-07-14, pedido do usuário — bloqueado ficava "totalmente preto" nas skills): `true` (armas, `ArsenalController.BuildScrollView`) tinge o ícone quase-preto + overlay 55%; `false` (skills) deixa o ícone na cor original, só com uma sombra por cima (0.96 de opacidade — começou em 0.5, depois 0.72 e 0.90, ajustado pro valor final no mesmo dia).
- **Resolução de tier**: `ArsenalController.ResolveWeaponSlot`/`ResolveSkillSlot` percorrem `profile.weapons`/`skills`, sobem a cadeia `previousTier` de cada item possuído até achar a raiz, e comparam com a família (raiz) da célula da grade — o maior `tier` encontrado entre os que batem vira o tier exibido, e o próprio `WeaponData`/`SkillData` possuído (ou a família/T1 se bloqueado) é o que o popup de detalhe mostra. Ícone: o do próprio item possuído (com fallback subindo `previousTier` se `icon` for nulo, mesmo padrão de `CharacterPanel`/`WeaponHandler.EquipSpecific`); se não possuído, mostra o ícone da própria família (T1), escurecido.
- **Grid**: `GridLayoutGroup` com `FixedColumnCount = 6` pras duas seções (ARMAS e SKILLS), `childAlignment = UpperCenter` (2026-07-14 — centraliza as células; era `UpperLeft`, sobrava vão vazio à direita) dentro de um único `ScrollRect` vertical — `VerticalLayoutGroup` + `ContentSizeFitter` no Content empilham Header+Grid de cada seção, cada `GridLayoutGroup` reporta sua própria altura preferida (baseada no nº de linhas) pro layout pai, sem precisar calcular manualmente.
...
|---|---|---|
| Assassin Guy | Aparece no grid de `02_SelectCharacter`, mas bloqueada (não é o `SelectedProfileHolder.currentProfile` atual) | Player1 option |
| Medieval Warrior | Sim — é quem está em `SelectedProfileHolder.currentProfile` hoje, único card clicável do grid | Player1 option |
| Medieval Warrior Girl | Aparece no grid (adicionada a `unlockedCharacters` em 2026-07-08 só pra isso), mas bloqueada — mesma regra acima | Oponente em `05_SelectOpponent` (`opponentCharacters`, hoje as 6 entradas do pool); **hardcoded como Player2 em `04_CombatScenePVP`**, inalterado |

**"Bloqueada" hoje = não ser o `currentProfile` atual** (2026-07-08) — não existe ainda um flag de unlock por personagem separado disso; `CharacterCardUI` compara `profile == SelectedProfileHolder.currentProfile` pra decidir cor cheia+clicável vs. portrait dessaturado+sem `Button`. Estar em `unlockedCharacters` só decide se o card aparece no grid, não se ele é clicável. To add a new character to the grid: create a `PlayerProfile` in `Assets/ScriptableObjects/PlayerProfiles/` and add it to `CharacterDatabase.unlockedCharacters`. To add a new opponent: add it to `CharacterDatabase.opponentCharacters` instead (não precisa estar em `unlockedCharacters` pra virar oponente).
...
(`MainMenuController.BuildPopup`, autocontido) oferecendo gastar diamante por +1 energia; sem
diamante suficiente, mensagem clara sugerindo a Loja e nenhuma batalha.

**Custo de "continuar jogando com energia zerada" é PROGRESSIVO por dia, POR PERSONAGEM
(2026-07-21, pedido do usuário — substitui o antigo `diamondCostToRefill` fixo)**: 1ª vez no dia
(hora do SERVIDOR, mesmo `ReadServerNowAsync` da regeneração acima) = `EnergySettings.
refillCostTier1` (10), 2ª = `refillCostTier2` (20), 3ª em diante = `refillCostTier3Plus` (40,
travado — não continua dobrando). Contador (`energyRefillPaymentsToday`/
`energyRefillLastPaymentTimestamp`, mesmo documento `characters/{characterId}` de
`energyCurrent`) reseta sozinho quando a data salva é de um dia anterior — sem job/Cloud Function
nenhum rodando à meia-noite. Pagar no Personagem A não afeta o contador do Personagem B (é o
mesmo documento por personagem de sempre). `EnergyService.GetRefillCostAsync` (só calcula, pro
popup mostrar o valor certo antes de confirmar) / `PayToRefillAsync` (gasta + incrementa contador
+ reabastece, recalculando o custo de novo por segurança contra corrida dia-virou-no-meio-tempo).
...
`PayToRefillAsync` (era `RefillOneAsync`, custo fixo — ver acima) só reabastece pra 1 agora — o
consumo desse 1 acontece no mesmo `OnCombatEnd` de sempre, não mais imediatamente em seguida no
clique.
...
- [x] Melhorar interface da tela de escolha de personagens — grid no estilo "grid de heróis" do Brawl Stars (`CharacterCardUI`: portrait com fundo colorido por personagem, tokens do `UITheme`; nome sobreposto numa faixa escura na base; Level+barra de XP compacta abaixo), fundo com gradiente `backgroundTop`/`backgroundBottom`. **Reescrita completa (2026-07-08)**: `CharacterSelectController` reescrito do zero — o antigo painel de preview lateral (`LeftPanel`: personagem duplicado instanciado via `SpawnPoint` + labels soltos de Level/Winrate/Xp/Jogos, nunca estilizados) e o antigo `DetailPanel` (abas Habilidades/Armas mortas, botões com `onClick` quebrado) foram **desativados por completo** (`m_IsActive: 0` — árvore de filhos aninhados não totalmente mapeada, mais seguro que deletar à mão via YAML); o grid (`RightPanel`) agora ocupa a tela inteira (redimensionado via código em `ResizeGridPanelFullScreen`, sem a faixa vazia reservada à esquerda que sobrava antes). `CanvasScaler` da cena corrigido de `Constant Pixel Size`/800×600 (inconsistente, provável causa raiz de parte da bagunça de layout) pra `Scale With Screen Size`/1920×1080, igual ao resto do projeto. Grid: `GridLayoutGroup` com `Constraint.FixedRowCount=3` (exatamente 3 personagens visíveis verticalmente) + `ScrollRect.horizontal=true/vertical=false` — revela novas colunas à direita conforme `CharacterDatabase.unlockedCharacters` cresce, sem ajuste manual. **Invertido pra scroll VERTICAL (2026-07-14, pedido do usuário — "igual do Arsenal")**: `GridLayoutGroup.Constraint.FixedColumnCount=3` (3 personagens por linha, preenchendo pra baixo) + `ScrollRect.horizontal=false/vertical=true`; `CharacterCardUI.CardWidth/CardHeight` reduzidos de 600/310 pra 500/259 (~5/6) pra 3 caberem na largura nova do Scroll View (1544.65px, `CharacterSelectController.GridScrollWidth`); Content passou do esquema "largura auto-fit, altura = Viewport" (scroll horizontal) pro esquema "largura = Viewport, altura auto-fit" (`ContentSizeFitter.verticalFit=PreferredSize`, mesmo padrão do Arsenal). `Viewport` já tinha `Image`+`Mask` próprios desde a montagem original da cena (raycastTarget=1) — diferente do bug corrigido no Arsenal, aqui o arraste em qualquer ponto da tela (não só em cima de um card) já funcionava de graça. Cards ordenados: habilitado (`== SelectedProfileHolder.currentProfile`) primeiro, bloqueados depois. **Painel de detalhe agora é o MESMO `CharacterPanel` do `01_MainMenu`** (não mais um painel ad-hoc) — `CharacterPanel` ganhou `showLevelXp`/`startHidden` (opcionais, default `false`, sem afetar o uso existente no menu) + `ShowSlideIn()`/`HideSlideOut()` (anima `Root.anchoredPosition` de/pra fora da tela) — instanciado já escondido, desliza a partir da direita ao clicar num card habilitado, headers/tabs/pips 100% reaproveitados. `Medieval Warrior Girl` segue em `CharacterDatabase.unlockedCharacters` (adicionada 2026-07-08, ver Characters acima) só pra aparecer bloqueada no grid — sem afetar o hardcode dela como Player2 em `04_CombatScenePVP`.
- [x] Criação do primeiro personagem masculino e feminino — coberto desde a introdução do 1º personagem masculino (Assassin Guy) e feminino (Medieval Warrior Girl); pipeline de importação (item abaixo) expandiu o roster pra 69 `PlayerProfile` no total, com vários personagens de cada gênero
...
- [x] Adicionar novos prefabs de personagens guardados — pipeline de importação incremental (`Assets/Editor/FemaleCharacterImportGenerator.cs`, `Tools > AutoArms`) + 13 personagens novos processados (ver CHARACTER_IMPORT_CHECKLIST.md), ocultos/travados até liberação manual via `isUnlockedForSelection`/`isPlayable`
...
- [x] **Arte chibi + retrato realista do personagem**: ao criar o personagem, ter duas versões visuais — o boneco chibi (estilo atual usado em combate/seleção) na frente, e uma versão mais realista do mesmo personagem ao fundo. Coberto via `PlayerProfile.splashArt` (2026-07-09) + `CharacterSelectController.UpdateFrameArt` — os 72 `PlayerProfile` têm `splashArt` preenchido (verificado 2026-07-14, nenhum com `{fileID: 0}`), exibido em tela cheia como fundo em `02_SelectCharacter` atrás do preview "ao vivo" do personagem chibi (`Portrait`/`BuildPortraitPreview`).
...
### Progresso
- Total: 144 tarefas | Concluídas: 53
- Contagem recalculada em 2026-07-21 (todo `- [ ]`/`- [x]` em CLAUDE.md Fase 0-3 + ROADMAP_FUTURO.md
  Fase 4-13, qualquer nível de indentação — SKILLS_SYSTEM.md/PETS.md/etc. têm suas próprias listas
  de implementação, não contam aqui). O total mudou de 146→144 em relação à última contagem
  (2026-07-16) — provável divergência acumulada de edições anteriores no ROADMAP_FUTURO.md sem
  recontagem, não algo desta sessão especificamente. Marcados `[x]` nesta rodada: Sistema de
  diamantes, Sistema de energia com limite diário, Precificação dos pacotes, Reset de Level Up
  (Fase 4) — ver ROADMAP_FUTURO.md pras notas de divergência de cada um (desenho final ficou
  diferente do texto original em vários). Recontar do zero (mesmo método) sempre que suspeitar de
  nova divergência.
- 2026-07-19: `HUD superior: moeda geral, diamante e energia` marcado `[x]` (+1 concluída).
- 2026-07-23: `Compra de energia e personagens com diamante` (Fase 4, ROADMAP_FUTURO.md) marcado
  `[x]` por completo — a metade "personagens" fechou com o sistema de compra/case opening
  (Cloud Function `purchaseCase`) (+1 concluída, 52→53). Contador não tinha sido atualizado nesta
  entrada quando o checkbox mudou; CHANGELOG.md já refletia 53 desde então.
